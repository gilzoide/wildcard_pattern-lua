...
describe("When building wildcards,", function()
    ...
    it("'[]' delimit character sets", function()
        local pattern = wildcard_pattern.from_wildcard("[01234567].txt")
        should_match(pattern, '0.txt')
        should_match(pattern, '1.txt')
        should_match(pattern, '4.txt')
        should_match(pattern, '7.txt')
        should_not_match(pattern, '9.txt')
        should_not_match(pattern, '0')
        should_not_match(pattern, '0.lua')
        should_not_match(pattern, '10.txt')
    end)

    it("unescaped '-' inside '[]' delimit character ranges", function()
    ...
    it("escaped '-' inside '[]' is just another character in the set", function()
        local pattern = wildcard_pattern.from_wildcard("[0\\-9].txt")
        should_match(pattern, '0.txt')
        should_match(pattern, '-.txt')
        should_match(pattern, '9.txt')
        should_not_match(pattern, '1.txt')
        should_not_match(pattern, '5.txt')
        should_not_match(pattern, '0')
        should_not_match(pattern, '0.lua')
        should_not_match(pattern, '10.txt')
    end)
